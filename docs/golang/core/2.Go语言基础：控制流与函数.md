---
title: 2、Go语言基础：控制流与函数
date: 2026-01-06
description: 深入理解Go语言的条件语句、循环语句和函数机制
---

# 2、Go语言基础：控制流与函数

本文档系统讲解Go语言的控制流语句和函数机制，是编写有逻辑的程序的基础。

## 🎯 本课学习目标

完成本课后，你将能够：
- ✅ 掌握条件语句（if）的使用
- ✅ 掌握循环语句（for）的各种形式
- ✅ 理解函数的定义、调用和参数传递
- ✅ 掌握defer语句的使用
- ✅ 理解匿名函数和闭包

---

## 一、条件语句（if）

### 1.1 基本if语句

```go
package main

import "fmt"

func main() {
    score := 85
    
    if score >= 90 {
        fmt.Println("优秀")
    } else if score >= 80 {
        fmt.Println("良好")
    } else {
        fmt.Println("需要努力")
    }
}
```

### 1.2 if的初始化语句

Go语言的if语句支持在条件前执行初始化语句：

```go
// 语法：if init; condition { }
if err := doSomething(); err != nil {
    fmt.Println("错误:", err)
}

// 常见用法：错误处理
if file, err := os.Open("file.txt"); err != nil {
    fmt.Println("打开文件失败:", err)
} else {
    defer file.Close()
    // 使用file
}
```

### 1.3 if的作用域

if语句中的初始化变量作用域仅限于if-else块：

```go
if x := 10; x > 5 {
    fmt.Println(x)  // 10
}
// fmt.Println(x)  // 编译错误：x未定义

// 如果需要在外层使用，在外层声明
var x int
if x = 10; x > 5 {
    fmt.Println(x)
}
fmt.Println(x)  // 10
```

### 1.4 常见模式

#### 1. 错误处理

```go
if err := function(); err != nil {
    return err
}
```

#### 2. 类型断言

```go
if value, ok := interfaceVar.(string); ok {
    fmt.Println("是字符串:", value)
}
```

#### 3. Map检查

```go
if value, exists := m["key"]; exists {
    fmt.Println("值:", value)
}
```

---

## 二、循环语句（for）

### 2.1 传统for循环

Go语言只有`for`一种循环语句，但支持多种形式：

```go
// 1. 传统三部分形式
for i := 0; i < 10; i++ {
    fmt.Println(i)
}

// 2. 类似while循环
i := 0
for i < 10 {
    fmt.Println(i)
    i++
}

// 3. 无限循环
for {
    // 需要break退出
    if condition {
        break
    }
}
```

### 2.2 for-range循环

用于遍历数组、切片、字符串、map和通道：

```go
// 遍历切片
nums := []int{1, 2, 3, 4, 5}
for index, value := range nums {
    fmt.Println(index, value)
}

// 只获取值
for _, value := range nums {
    fmt.Println(value)
}

// 只获取索引
for index := range nums {
    fmt.Println(index)
}

// 遍历字符串（按rune）
str := "Hello"
for i, char := range str {
    fmt.Printf("%d: %c\n", i, char)
}

// 遍历map
m := map[string]int{"a": 1, "b": 2}
for key, value := range m {
    fmt.Println(key, value)
}
```

### 2.3 循环控制

#### break和continue

```go
// break：跳出循环
for i := 0; i < 10; i++ {
    if i == 5 {
        break  // 跳出循环
    }
    fmt.Println(i)  // 0, 1, 2, 3, 4
}

// continue：跳过本次循环
for i := 0; i < 10; i++ {
    if i%2 == 0 {
        continue  // 跳过偶数
    }
    fmt.Println(i)  // 1, 3, 5, 7, 9
}
```

#### 标签（Label）

使用标签可以跳出外层循环：

```go
outer:
for i := 0; i < 3; i++ {
    for j := 0; j < 3; j++ {
        if i == 1 && j == 1 {
            break outer  // 跳出外层循环
        }
        fmt.Println(i, j)
    }
}
```

### 2.4 for循环的常见模式

#### 1. 遍历并修改切片

```go
nums := []int{1, 2, 3, 4, 5}
for i := range nums {
    nums[i] *= 2  // 修改原切片
}
fmt.Println(nums)  // [2 4 6 8 10]
```

#### 2. 查找元素

```go
nums := []int{1, 2, 3, 4, 5}
target := 3
found := false

for _, num := range nums {
    if num == target {
        found = true
        break
    }
}
```

#### 3. 累加/聚合

```go
nums := []int{1, 2, 3, 4, 5}
sum := 0
for _, num := range nums {
    sum += num
}
fmt.Println(sum)  // 15
```

---

## 三、函数（Function）

### 3.1 函数的基本定义

```go
package main

import "fmt"

// 基本函数
func greet(name string) {
    fmt.Println("Hello,", name)
}

// 带返回值的函数
func add(a, b int) int {
    return a + b
}

// 多个返回值
func divide(a, b float64) (float64, error) {
    if b == 0 {
        return 0, fmt.Errorf("除数不能为0")
    }
    return a / b, nil
}

func main() {
    greet("Alice")
    result := add(3, 5)
    fmt.Println(result)  // 8
    
    quotient, err := divide(10, 2)
    if err != nil {
        fmt.Println("错误:", err)
    } else {
        fmt.Println("结果:", quotient)  // 5
    }
}
```

### 3.2 函数签名

函数签名包括：函数名、参数列表、返回值列表。

```go
// 签名：func(int, int) int
func add(a, b int) int {
    return a + b
}

// 签名：func(string, ...int) (int, error)
func sum(prefix string, numbers ...int) (int, error) {
    // ...
}
```

### 3.3 命名返回值

Go语言支持命名返回值，可以在函数体中直接使用：

```go
// 命名返回值
func calculate(x, y int) (sum int, product int) {
    sum = x + y        // 直接赋值，无需声明
    product = x * y
    return             // 自动返回命名返回值
}

// 等价于
func calculate2(x, y int) (int, int) {
    sum := x + y
    product := x * y
    return sum, product
}
```

**注意**：命名返回值会被初始化为零值，即使没有显式赋值。

### 3.4 可变参数（Variadic）

使用`...`定义可变参数：

```go
package main

import "fmt"

// 可变参数必须是最后一个参数
func sum(numbers ...int) int {
    total := 0
    for _, num := range numbers {
        total += num
    }
    return total
}

func main() {
    fmt.Println(sum(1, 2, 3))        // 6
    fmt.Println(sum(1, 2, 3, 4, 5))  // 15
    
    // 传递切片
    nums := []int{1, 2, 3}
    fmt.Println(sum(nums...))        // 6
}
```

### 3.5 函数作为值

在Go语言中，函数是**一等公民**，可以作为值传递：

```go
package main

import "fmt"

// 函数类型
type Operation func(int, int) int

func add(a, b int) int {
    return a + b
}

func multiply(a, b int) int {
    return a * b
}

// 函数作为参数
func calculate(a, b int, op Operation) int {
    return op(a, b)
}

func main() {
    result1 := calculate(3, 4, add)       // 7
    result2 := calculate(3, 4, multiply)  // 12
    
    fmt.Println(result1, result2)
}
```

### 3.6 匿名函数和闭包

#### 匿名函数

```go
// 立即执行
func() {
    fmt.Println("匿名函数")
}()

// 赋值给变量
add := func(a, b int) int {
    return a + b
}
result := add(3, 5)
```

#### 闭包（Closure）

闭包可以捕获外部变量：

```go
package main

import "fmt"

func counter() func() int {
    count := 0
    return func() int {
        count++
        return count
    }
}

func main() {
    c1 := counter()
    fmt.Println(c1())  // 1
    fmt.Println(c1())  // 2
    fmt.Println(c1())  // 3
    
    c2 := counter()
    fmt.Println(c2())  // 1（独立的闭包）
}
```

#### 闭包的常见陷阱

```go
// 陷阱：循环变量问题（Go 1.22已修复for-range）
var funcs []func()
for i := 0; i < 3; i++ {
    // 错误：所有闭包共享同一个i
    funcs = append(funcs, func() {
        fmt.Println(i)  // 都输出3
    })
}

// 正确：创建局部副本
for i := 0; i < 3; i++ {
    i := i  // 创建局部副本
    funcs = append(funcs, func() {
        fmt.Println(i)  // 0, 1, 2
    })
}
```

### 3.7 defer语句

`defer`用于延迟执行函数，常用于资源清理：

```go
package main

import "fmt"

func main() {
    defer fmt.Println("最后执行")
    fmt.Println("先执行")
    fmt.Println("中间执行")
    // 输出：
    // 先执行
    // 中间执行
    // 最后执行
}
```

#### defer的执行顺序

多个defer按**后进先出（LIFO）**顺序执行：

```go
func main() {
    defer fmt.Println("1")
    defer fmt.Println("2")
    defer fmt.Println("3")
    // 输出：3, 2, 1
}
```

#### defer的常见用途

```go
// 1. 关闭文件
file, err := os.Open("file.txt")
if err != nil {
    return err
}
defer file.Close()  // 确保文件被关闭

// 2. 解锁
mu.Lock()
defer mu.Unlock()

// 3. 恢复panic
defer func() {
    if r := recover(); r != nil {
        fmt.Println("恢复:", r)
    }
}()
```

#### defer的参数求值

defer的参数在**声明时**求值，而非执行时：

```go
func main() {
    x := 1
    defer fmt.Println(x)  // 输出1（不是2）
    x = 2
}

// 如果需要使用执行时的值
func main() {
    x := 1
    defer func() {
        fmt.Println(x)  // 输出2
    }()
    x = 2
}
```

### 3.8 方法（Method）

方法是有接收者的函数：

```go
type Rectangle struct {
    Width, Height float64
}

// 值接收者方法
func (r Rectangle) Area() float64 {
    return r.Width * r.Height
}

// 指针接收者方法
func (r *Rectangle) Scale(factor float64) {
    r.Width *= factor
    r.Height *= factor
}
```

### 3.9 函数的最佳实践

```go
// ✅ 错误处理：返回error
func doSomething() error {
    // ...
    return nil
}

// ✅ 使用命名返回值提高可读性
func parse(input string) (result string, err error) {
    // ...
}

// ✅ 使用defer确保资源清理
func readFile(filename string) error {
    file, err := os.Open(filename)
    if err != nil {
        return err
    }
    defer file.Close()
    // ...
}

// ✅ 函数应该短小精悍，单一职责
func calculateTotal(items []Item) float64 {
    // ...
}

// ❌ 避免过长的函数
// ❌ 避免过多的参数（超过3-4个考虑使用结构体）
```

---

## 四、综合示例

```go
package main

import (
    "fmt"
    "errors"
)

// 计算器结构体
type Calculator struct {
    result float64
}

// 构造函数
func NewCalculator() *Calculator {
    return &Calculator{result: 0}
}

// 加法
func (c *Calculator) Add(value float64) {
    c.result += value
}

// 减法
func (c *Calculator) Subtract(value float64) {
    c.result -= value
}

// 乘法
func (c *Calculator) Multiply(value float64) {
    c.result *= value
}

// 除法
func (c *Calculator) Divide(value float64) error {
    if value == 0 {
        return errors.New("除数不能为0")
    }
    c.result /= value
    return nil
}

// 获取结果
func (c *Calculator) GetResult() float64 {
    return c.result
}

// 重置
func (c *Calculator) Reset() {
    c.result = 0
}

func main() {
    calc := NewCalculator()
    
    // 执行一系列计算
    operations := []struct {
        op    string
        value float64
    }{
        {"add", 10},
        {"multiply", 2},
        {"subtract", 5},
        {"divide", 3},
    }
    
    for _, op := range operations {
        switch op.op {
        case "add":
            calc.Add(op.value)
        case "subtract":
            calc.Subtract(op.value)
        case "multiply":
            calc.Multiply(op.value)
        case "divide":
            if err := calc.Divide(op.value); err != nil {
                fmt.Println("错误:", err)
                return
            }
        }
        fmt.Printf("执行 %s %.2f，当前结果: %.2f\n", op.op, op.value, calc.GetResult())
    }
    
    fmt.Printf("\n最终结果: %.2f\n", calc.GetResult())
}
```

---

## 总结

1. **if语句**：支持初始化语句，作用域限制在if-else块内，常用于错误处理
2. **for循环**：唯一的循环语句，支持多种形式，注意循环变量的作用域
3. **函数**：一等公民，支持多返回值、可变参数、闭包，使用defer进行资源清理
4. **defer语句**：按LIFO顺序执行，参数在声明时求值，常用于资源清理
5. **闭包**：可以捕获外部变量，注意循环变量的陷阱

掌握这些核心特性，是编写高质量Go代码的基础。在实际开发中，要特别注意：
- for循环变量的作用域陷阱
- defer的执行时机和参数求值
- 函数的错误处理模式
- 闭包的正确使用方式
